...
const Tower: React.FC<TowerProps> = ({ 
    ...
    const [aiStatus, setAiStatus] = useState<'Lyssnar' | 'Tänker' | 'Talar' | 'Vilar'>('Vilar');
    ...
    useEffect(() => {
        ...
        const render = (timestamp: number) => {
            ...
            // AI Status Logic
            let currentStatus: 'Lyssnar' | 'Tänker' | 'Talar' | 'Vilar' = 'Vilar';
            if (isSpeakerActive || data.queueLength > 0) {
                currentStatus = 'Talar';
            } else if (isAiInActive || isAiOutActive || data.busyRemaining > 0) {
                currentStatus = 'Tänker';
            } else if (isMicActive || data.activeMode === 'translate') {
                currentStatus = 'Lyssnar';
            }
                ...
                <div className="flex flex-col md:flex-row h-full w-full">
                    <div className="flex-1 overflow-y-auto bg-slate-900/30 border-r border-slate-800 p-4 md:p-8 scrollbar-hide">
                        <div className="max-w-xl mx-auto space-y-8 pb-20">
                            ...
                            {/* AI Status Indicator */}
                            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 flex flex-col items-center justify-center shadow-2xl relative overflow-hidden">
                                <div className="absolute inset-0 bg-gradient-to-b from-transparent to-slate-950/50 pointer-events-none" />
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 z-10">AI Status</h3>
                                <div className="flex items-center gap-6 z-10">
                                    <div className={`flex flex-col items-center gap-2 transition-opacity duration-300 ${aiStatus === 'Lyssnar' ? 'opacity-100' : 'opacity-30'}`}>
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${aiStatus === 'Lyssnar' ? 'bg-green-500/20 text-green-400 shadow-[0_0_30px_rgba(34,197,94,0.4)]' : 'bg-slate-800 text-slate-500'}`}>
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
                                        </div>
                                        <span className="text-xs font-bold">Lyssnar</span>
                                    </div>
                                    <div className="w-8 h-px bg-slate-700" />
                                    <div className={`flex flex-col items-center gap-2 transition-opacity duration-300 ${aiStatus === 'Tänker' ? 'opacity-100' : 'opacity-30'}`}>
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${aiStatus === 'Tänker' ? 'bg-fuchsia-500/20 text-fuchsia-400 shadow-[0_0_30px_rgba(217,70,239,0.4)] animate-pulse' : 'bg-slate-800 text-slate-500'}`}>
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                        </div>
                                        <span className="text-xs font-bold">Tänker</span>
                                    </div>
                                    <div className="w-8 h-px bg-slate-700" />
                                    <div className={`flex flex-col items-center gap-2 transition-opacity duration-300 ${aiStatus === 'Talar' ? 'opacity-100' : 'opacity-30'}`}>
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${aiStatus === 'Talar' ? 'bg-indigo-500/20 text-indigo-400 shadow-[0_0_30px_rgba(99,102,241,0.4)]' : 'bg-slate-800 text-slate-500'}`}>
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg>
                                        </div>
                                        <span className="text-xs font-bold">Talar</span>
                                    </div>
                                </div>
                            </div>
